$(function() {
    ...
    $("#talk").on('click', 'a[id^=delete-post-]', function(){
     ...
     // Like on click
      $("#talk").on('click','input[id^=count-]',function(){
    ...
    function create_post() {
        ...
        $.ajax({
            ...
            success : function(json)  {
                ...
                $("#talk").prepend("<l><strong>"+json.text+"</strong>-<em> "+json.author+"</em> - <span> "+json.created+
                 ' <input id="count-'+json.postpk+'" type="button" class="like" name="'+json.postpk+'" value="Like">'+
                '<a id="count'+json.postpk+'"> :'+ json.like +'</a>'
                +    "</span>  <a id='delete-post-"+json.postpk+"'>delete me</a></l><br>");
                console.log("success"); // another sanity check
            },
            // handle a non-successful response
            error : function(xhr,errmsg,err) {
                $('#results').html("<div class='alert-box alert radius' data-alert>Oops! We have encountered an error: "+errmsg+
                    " &times;</div>"); // add the error to the dom
...
//                      $("#talk").hide();
                        $("#delete-post-"+post_primary_key).parent('l').hide()
